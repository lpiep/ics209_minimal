...
curr_data_cleaned <- curr_data %>% 
  transmute(
    ...
    ics_wildfire_fatalities_tot = as.integer(FATALITIES), 
    ics_wildfire_injuries_tot = pmax(as.numeric(INJURIES_TO_DATE), as.numeric(INJURIES_THIS_REP_PERIOD), na.rm = TRUE), 
    ics_name = INCIDENT_NAME,
    ics_wildfire_area = as.numeric(DISP_INC_AREA) * 0.00404686, # convert to km^2
    ics_wildfire_struct_destroyed = as.numeric(STRUCTURES_DESTROYED_COUNT),
    ics_wildfire_struct_threatened = as.numeric(STRUCTURES_THREATENED_COUNT),
    ics_wildfire_cost = as.numeric(PROJECTED_FINAL_IM_COST),
